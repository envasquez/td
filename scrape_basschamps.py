...
def get_tournament_links(urls):
    ...
    for url in urls:
        ...
        pattern = re.compile(r"^results\.cfm\?tournament_id=\d+&type=team&yearSelected=\d+$")
...
def get_tournament_results(url):
    ...
    def _parse_prize_cell(c):
        ...
        prize_patterns = [
            ...
            (re.compile(r'skeeter\s*sx190.*yamaha\s*150.*\$1000.*big\s*bass', re.IGNORECASE), 23900),
            (re.compile(r'skeeter\s*sx190.*yamaha\s*150.*\$5000.*bonus', re.IGNORECASE), 27900),
            (re.compile(r'skeeter\s*sx190.*yamaha\s*150.*\$5000.*bonus.*\$1000.*big\s*bass', re.IGNORECASE), 28900),
            ...
            (re.compile(r'skeeter\s*sx200.*\$5000.*bonus.*\$1000.*big\s*bass', re.IGNORECASE), 27000),
            (re.compile(r'\$2750.*\$5000.*bonus', re.IGNORECASE), 7750),
            (re.compile(r'yamaha150.*real money.*\$300.*big\s*bass', re.IGNORECASE), 24200),
            (re.compile(r'skeeter\s*zx\s*20.*yamaha\s*225.*sho.*\$200.*sure[-\s]*life', re.IGNORECASE), 35000),
        ]
    ...
    def _parse_results_table():
        ...
        for row in soup.find_all('tr'):
            ...
            results.append({
                "place": place,
                "skeeter_boat": True if img_tag and 'skeeter' in img_tag.get('src', '').lower() else False,
                "angler1": angler1,
    ...
    try:
        ...
        metadata = {"Date": None, "Region": None, "Tournament": None, "Tournament Trail": None}
        for row in soup.find_all("tr"):
